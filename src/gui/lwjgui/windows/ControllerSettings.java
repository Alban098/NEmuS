...
/**
 * This class represent the Controller Settings Window
 */
public class ControllerSettings extends Application implements Initializable {
    ...
    /**
     * Create a new instance of ControllerSettings
     */
    ...
    /**
     * Does an instance of ControllerSettings exist
     *
    ...
    private void editControls(MouseEvent event) {
        if (keyboardEdited == null) {
            ...
            long window = openWindow("Press a Key");
    ...
    /**
     * Open a small popup window to register inputs
     *
     * @param title the window title
     * @return the window id
     */
    private long openWindow(String title) {
        long window = glfwCreateWindow(300, 1, title, NULL, NULL);
        if (window == NULL)
    ...
    private void editControlsGamepad(MouseEvent event) {
        ...
        if (gamepadEdited == null && glfwJoystickPresent(GLFW_JOYSTICK_1)) {
            ...
            glfwDefaultWindowHints();
            glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE);
            long window = openWindow("Press a Button");

            //We wait for the user to press a button
            while ((key = emulator.getInputMapper().getButtonPressed()) == null)
                glfwPollEvents();
